...
public:
	...
	ID3D11RenderTargetView *const *GetRenderTargetPointer(std::size_t arrayIndex = 0) const { return &mRenderTargetElements[arrayIndex].p; }

	ID3D11UnorderedAccessView *GetUnorderedAccess(std::size_t arrayIndex = 0) const { return mUnorderedAccessElements[arrayIndex]; }
	ID3D11UnorderedAccessView *const *GetUnorderedAccessPointer(std::size_t arrayIndex = 0) const { return &mUnorderedAccessElements[arrayIndex].p; }
	...
	ID3D11ShaderResourceView *const *GetShaderResourcePointer(std::size_t arrayIndex) const { return &mShaderResourceElements[arrayIndex].p; }
...
private:
	...
	std::vector<CComPtr<ID3D11RenderTargetView>> mRenderTargetElements;
	std::vector<CComPtr<ID3D11UnorderedAccessView>> mUnorderedAccessElements;
	std::vector<CComPtr<ID3D11ShaderResourceView>> mShaderResourceElements;
