...
bool DirectX11Texture2DHelper::InternalConstruct(ID3D11Device* d3dDevice, int width, int height, DXGI_FORMAT format, UINT bindFlags, int mipLevels, int arraySize, int sampleCount, int sampleQuality, D3D11_RTV_DIMENSION rtvDimension, D3D11_UAV_DIMENSION uavDimension, D3D11_SRV_DIMENSION srvDimension)
{
	...
	if(FAILED(d3dDevice->CreateTexture2D(&textureDesc, 0, &mTexture)))
		return true;
	...
	if(bindFlags & D3D11_BIND_RENDER_TARGET)
	{
		...
		for(int i = 0; i < arraySize; ++i)
		{
			...
			if(FAILED(d3dDevice->CreateRenderTargetView(mTexture, &textureRTVDesc, &mRenderTargetElements[i])))
				return true;
		}
	}
	...
	if(bindFlags & D3D11_BIND_UNORDERED_ACCESS)
	{
		...
		for(int i = 0; i < arraySize; ++i)
		{
			...
			if(FAILED(d3dDevice->CreateUnorderedAccessView(mTexture, &textureUAVDesc, &mUnorderedAccessElements[i])))
				return true;
		}
	}
	...
	if(bindFlags & D3D11_BIND_SHADER_RESOURCE)
	{
		...
		for(int i = 0; i < arraySize; ++i)
		{
			...
			if(FAILED(d3dDevice->CreateShaderResourceView(mTexture, &srvElementDesc, &mShaderResourceElements[i])))
				return true;
		}
	}
