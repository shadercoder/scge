...
bool DirectX11VertexShader::Load()
{
	...
	if(FAILED(D3DX11CompileFromFile(fileName.c_str(), mResourceData->mShaderDefines, nullptr, mResourceData->mFunctionName.c_str(), "vs_5_0", shaderFlags, 0, nullptr, mShaderBuffer.getModifiablePointer(), errorMessage.getModifiablePointer(), nullptr)))
	{
...
{
	if(FAILED(mResourceData->mD3D11Device->CreateVertexShader(mShaderBuffer->GetBufferPointer(), mShaderBuffer->GetBufferSize(), nullptr, mVertexShader.getModifiablePointer())))
	{
	...
	if(FAILED(mResourceData->mD3D11Device->CreateInputLayout(elements, numElements, mShaderBuffer->GetBufferPointer(), mShaderBuffer->GetBufferSize(), mVertexLayout.getModifiablePointer())))
	{
