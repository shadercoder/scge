...
void Matrix3::SingularValueDecomposition(Matrix3& kL, Vector3& kS, Matrix3& kR) const
{
	...
	for (unsigned int i = 0; i < 32; i++)
	{
		...
		bool bTest1 = (Math::Abs(kA[0][1]) <= Math::Limits::FloatEpsilon*(Math::Abs(kA[0][0])+Math::Abs(kA[1][1])));
		bool bTest2 = (Math::Abs(kA[1][2]) <= Math::Limits::FloatEpsilon*(Math::Abs(kA[1][1])+Math::Abs(kA[2][2])));
		if ( bTest1 )
		{
			if ( bTest2 )
			{
...
float Matrix3::MaxCubicRoot (float afCoeff[3])
{
	...
	if ( fDiscr <= Math::Limits::FloatEpsilon )
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if ( Math::Abs(fPoly) <= Math::Limits::FloatEpsilon )
			return fX;
...
void Matrix3::ToAxisAngle(Vector3& rkAxis, float& rfRadians) const
{
	...
	if(rfRadians > 0.0f)
	{
		if(rfRadians < Math::Constants::Pi)
		{
...
EulerAngles Matrix3::ToEulerAnglesXYZ() const
{
	...
	float rfPAngle = Math::ASin(m[0][2]);
	float rfYAngle = 0.0f;
	float rfRAngle = 0.0f;

	if(rfPAngle < Math::Constants::HalfPi)
	{
		if(rfPAngle > -Math::Constants::HalfPi)
		{
			rfYAngle = Math::ATan2(-m[1][2],m[2][2]);
			rfRAngle = Math::ATan2(-m[0][1],m[0][0]);
			return EulerAngles(rfYAngle, rfPAngle, rfRAngle);
		}
		else
		{
...
void Matrix3::FromEulerAnglesXYZ(const EulerAngles& angles)
{
	float fCos, fSin;

	fCos = Math::Cos(angles.x);
	fSin = Math::Sin(angles.x);
	Matrix3 kXMat(1.0, 0.0, 0.0, 0.0, fCos, -fSin, 0.0, fSin, fCos);

	fCos = Math::Cos(angles.y);
	fSin = Math::Sin(angles.y);
	Matrix3 kYMat(fCos, 0.0, fSin, 0.0, 1.0, 0.0, -fSin, 0.0, fCos);

	fCos = Math::Cos(angles.z);
	fSin = Math::Sin(angles.z);
...
void Matrix3::Tridiagonal(float afDiag[3], float afSubDiag[3])
{
	...
	afDiag[0] = fA;
	afSubDiag[2] = 0.0;
	if ( Math::Abs(fC) >= Math::Limits::FloatEpsilon )
	{
